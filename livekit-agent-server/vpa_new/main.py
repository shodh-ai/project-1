...
try:
    ...
    logger.error(
        "Please install the missing packages: pip install 'livekit-agents[deepgram,openai,silero,turn-detector]~=1.0' 'livekit-plugins-noise-cancellation~=0.2' python-dotenv"
    )
    sys.exit(1)
...
for var in required_vars:
    ...
    if var == "DEEPGRAM_API_KEY":
        logger.info(
            f"DEEPGRAM_API_KEY: {value[:8]}...{value[-4:]} (length: {len(value)})"
        )
...
GLOBAL_INSTRUCTIONS = (
    "You are a helpful voice AI assistant. Be concise but thorough in your responses."
)
...
if __name__ == "__main__":
    ...
    while i < len(sys.argv):
        arg = sys.argv[i]
        if arg in ["--page-path", "--tts-model", "--temperature"] and i + 1 < len(
            sys.argv
        ):
